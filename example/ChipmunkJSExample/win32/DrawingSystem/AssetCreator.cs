...
namespace ChipmunkExample
{
    ...
    public class AssetCreator
    {
        ...
        public static Vector2 CalculateOrigin(cpBody b)
        {
        ...
        public Texture2D TextureFromShape(cpShape shape, MaterialType type, Color color, float materialScale)
        {
            switch (shape.shapeType)
            {
                case cpShapeType.Circle:
                    return CircleTexture((shape as cpCircleShape).Radius, type, color, materialScale);
                case cpShapeType.Polygon:
                    return TextureFromVertices(((cpPolyShape)shape).verts, type, color, materialScale);
                default:
                    throw new NotSupportedException("The specified shape type is not supported.");
            }
        }

        public Texture2D TextureFromVertices(List<cpVect> vertices, MaterialType type, Color color, float materialScale)
        {
            // copy vertices
            cpVect verts = new cpVect(vertices);
            ...
            {
                decomposedVerts = new List<cpVect>();
