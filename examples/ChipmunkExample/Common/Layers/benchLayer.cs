...
namespace ChipmunkExample
{
	class benchLayer : ChipmunkDemoLayer
	{
		...
		protected override void AddedToScene()
		{
			...
			Position = windowSize.Center;  //new CCPoint(150, 150);
	...
	new cpVect(456.76f, 382.00f), new cpVect(432.95f, 389.00f),new cpVect(417.00f, 411.32f), new cpVect(373.00f, 433.19f), new cpVect(361.00f, 430.02f), new cpVect(350.00f, 425.07f),
		...
		static public void add_box(cpSpace space, int index, float size)
		{
			...
			cpShape shape = space.AddShape(cpPolyShape.BoxShape(body, size - bevel * 2f, size - bevel * 2f,0.0f));
			(shape as cpPolyShape).r = bevel;
			shape.SetElasticity(0.0f); shape.SetFriction(0.9f);
		}
		...
		static public void add_hexagon(cpSpace space, int index, float radius)
		{
			...
			for (int i = 0; i < 6; i++)
			{
				float angle = -CCMathHelper.Pi * 2.0f * i / 6.0f;
				var vert = cpVect.cpvmult(new cpVect(CCMathHelper.Cos(angle), CCMathHelper.Sin(angle)), radius - bevel);
			...
			cpBody body = space.AddBody(new cpBody(mass, cp.momentForPoly(mass, hexagon, cpVect.Zero)));
			body.SetPosition(cpVect.cpvmult(cp.frand_unit_circle(), 180.0f));
			cpShape shape = space.AddShape(new cpPolyShape(body, hexagon,0.0f));
		...
		static public cpSpace SetupSpace_simpleTerrain(cpSpace space)
		{
			...
			for (int i = 0; i < (simple_terrain_verts.Length - 1); i++)
			{
				...
				space.AddShape(new cpSegmentShape(space.StaticBody, cpVect.cpvadd(a, offset), cpVect.cpvadd(b, offset), 0.0f));
			}
	...
	new cpVect(603.00f, 475.63f), new cpVect(590.96f, 479.00f), 
		...
		static public cpSpace ComplexTerrainHexagons_1000(cpSpace space)
		{
			...
			{
				float angle = -CCMathHelper.Pi * 2.0f * i / 6.0f;
				var tmp = cpVect.cpvmult(new cpVect(CCMathHelper.Cos(angle), CCMathHelper.Sin(angle)), radius - bevel);
				hexagon[i] = tmp.x;
				hexagon[i + 1] = tmp.y;
			}
	...
	new cpVect(404.42f,  39.00f), new cpVect(408.00f,  33.88f), new cpVect(415.00f,  29.31f), new cpVect(429.00f,  26.45f), new cpVect(455.00f,  28.77f), new cpVect(470.00f,  33.81f), new cpVect(482.00f,  42.16f), new cpVect(494.00f,  46.85f), new cpVect(499.65f,  36.00f), new cpVect(513.00f,  25.95f), new cpVect(529.00f,  22.42f), new cpVect(537.18f,  23.00f), 
		...
		static public cpSpace BouncyTerrainCircles_500(cpSpace space)
		{
			...
			for (int i = 0; i < (bouncy_terrain_verts.Length - 1); i++)
			{
				...
				cpShape shape = space.AddShape(new cpSegmentShape(space.StaticBody, cpVect.cpvadd(a, offset), cpVect.cpvadd(b, offset), 0.0f));
		...
		static public cpSpace BouncyTerrainHexagons_500(cpSpace space)
		{
			...
			{
				float angle = -CCMathHelper.Pi * 2.0f * i / 6.0f;
				tmp = cpVect.cpvmult(new cpVect(CCMathHelper.Cos(angle), CCMathHelper.Sin(angle)), radius - bevel);
				hexagon[i] = tmp.x;
				hexagon[i + 1] = tmp.y;
			}
		...
		static cpSpace NoCollide(cpSpace space)
		{
			...
			space.AddCollisionHandler("2", "2", (a, s, f) => NoCollide_begin(a, s, null), null, null, null);

			float radius = 4.5f;

			space.AddShape(new cpSegmentShape(space.StaticBody, new cpVect(-330 - radius, -250 - radius), new cpVect(330 + radius, -250 - radius), 0.0f)).SetElasticity(1.0f);
			space.AddShape(new cpSegmentShape(space.StaticBody, new cpVect(330 + radius, 250 + radius), new cpVect(330 + radius, -250 - radius), 0.0f)).SetElasticity(1.0f);
			space.AddShape(new cpSegmentShape(space.StaticBody, new cpVect(330 + radius, 250 + radius), new cpVect(-330 - radius, 250 + radius), 0.0f)).SetElasticity(1.0f);
			space.AddShape(new cpSegmentShape(space.StaticBody, new cpVect(-330 - radius, -250 - radius), new cpVect(-330 - radius, 250 + radius), 0.0f)).SetElasticity(1.0f);
			...
			for (int x = -320; x <= 320; x += 20)
			{
				...
				{
					space.AddShape(new cpCircleShape(space.StaticBody, radius, new cpVect(x, y)));
				}
			}
