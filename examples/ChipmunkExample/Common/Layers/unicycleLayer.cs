	...
	class unicycleLayer : ChipmunkDemoLayer
	{
		...
		protected override void AddedToScene()
		{
...

			{
				...
				balance_body.SetPosition(new cpVect(0.0f, wheel_body.Position.y + cog_offset));
			...
			cpVect anchorA = balance_body.WorldToLocal(wheel_body.Position);
			...
			motor.preSolve = (s) => motor_preSolve(motor, s);
		...
		void motor_preSolve(cpConstraint motor, cpSpace space)
		{
			...
			float max_v = 500.0f;
			float target_v = cp.cpfclamp(cp.bias_coef(0.5f, dt / 1.2f) * (target_x - balance_body.Position.x) / dt, -max_v, max_v);
			float error_v = (target_v - balance_body.Vel.x);
			...
			float angular_diff = (float)Math.Asin(cpVect.cpvcross(balance_body.Rotation, cpVect.ForAngle(target_a)));
			float target_w = cp.bias_coef(0.1f, dt / 0.4f) * (angular_diff) / dt;

			float max_rate = 50.0f;
			float rate = cp.cpfclamp(wheel_body.GetAngVel() + balance_body.GetAngVel() - target_w, -max_rate, max_rate);
			motor.SetRate(cp.cpfclamp(rate, -max_rate, max_rate));
			motor.maxForce = 8.0e4f;
		}
		...
		protected override void Draw()
		{
			...
			{
				m_debugDraw.Begin();
				//m_debugDraw.DrawSegment(paint.point1, paint.point2, cpColor.Red);
				m_debugDraw.End();
			}
