	...
	public class ChipmunkDemoLayer : CCLayer
	{
		...
		public void addFloor()
		{
			var floor = space.AddShape(
				new cpSegmentShape(space.StaticBody, new cpVect(-Width * .5f, -Height * .5f), new cpVect(Width * .5f, -Height * .5f),
					0f));
		...
		public void addWalls()
		{
			...
			var wall1 = space.AddShape(new cpSegmentShape(space.StaticBody, new cpVect(-Width * .5f, -Height * .5f), new cpVect(-Width * .5f, Height * .5f), 0));
			wall1.SetElasticity(1);
			wall1.SetFriction(1);
			wall1.SetFilter(NOT_GRABBABLE_FILTER);

			var wall2 = space.AddShape(new cpSegmentShape(space.StaticBody, new cpVect(Width * .5f, -Height * .5f), new cpVect(Width * .5f, Height * .5f), 0));
		...
		public virtual void OnTouchesBegan(List<CCTouch> touches, CCEvent e)
		{
			...
			if (!CCMouse.Instance.HasBodyJoined)
			{
				...
				var shape = space.NearestPointQuery(
					CCMouse.Instance.Position,0,cp.ALL_LAYERS,cp.NO_GROUP);
